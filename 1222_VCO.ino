...
//sets the accuracy of the tuning. default is 0.5%. smaller % = more accurate but harder to tune and can be jumpy due to frequency fluctuations.
float margin = 0.5/100; //margin of accuracy 
...
void setDisplay(String note, int noteOutBy) {
   ...
   for (int n=0;n<11;n++) {
      digitalWrite(ledPins[n],  displaySet[n] == '0' ? LOW : HIGH);
    }
}
...
void getNote() {
  ...
  for (int n = 0; n < NUM_NOTES; n++) {
    ...
    if ((frequency > noteFrequ2 - ((noteFrequ2 - noteFrequ1)/2)) && (frequency <= noteFrequ3 - ((noteFrequ3 - noteFrequ2)/2))) {       
      ...
      if ((noteOutBy < 0  &&  noteOutBy + (((int)(noteFrequ2*margin)+0.5)*2) >= 0) || (noteOutBy > 0   &&    noteOutBy - (((int)(noteFrequ2*margin)+0.5)*2) <= 0)) {  
...
void loop(){
  //frequency = (38462*10)/period;  //ORIGINAL frequency timer rate of 38462
  frequency = ((38745*10)/period); //found 38745 to be more accurate when when calibrating with external tuner
  getNote();
